...
class EncoderTextBERT(nn.Module):
    def __init__(self, config, order_embeddings=False, mean=True, post_transformer_layers=0):
        ...
        bert_config = BertConfig.from_pretrained("/public12_data/hwx/TGDT/models/bert/",
                                                 output_hidden_states=True,
                                                 num_hidden_layers=config['text-model']['extraction-hidden-layer'])
        bert_model = BertModel.from_pretrained("/public12_data/hwx/TGDT/models/bert/", config=bert_config)
        self.order_embeddings = order_embeddings
        ...
        if not self.preextracted:
            self.tokenizer = BertTokenizer.from_pretrained("/public12_data/hwx/TGDT/models/bert/")
    ...
    def forward(self, x, lengths):
        ...
        if self.mean:
            ...
            # from the last layer take only the first word

            x = outputs[:, 0, :]  #提取特殊向量cls作为文本的全局表示
