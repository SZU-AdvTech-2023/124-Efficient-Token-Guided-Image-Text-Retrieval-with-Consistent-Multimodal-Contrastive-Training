...
class AlignmentContrastiveLoss(Contrastive):
    ...
    def forward(self, im_set, s_seq, im_len, s_len):
        ...
        aggr_similarity = alignments.max(2)[0].sum(2)*10

        # for index,lens in enumerate(s_len):
        #     aggr_similarity[:,index]=aggr_similarity[:,index]/lens
...
class ContrastiveLoss(Contrastive):
    ...
    def forward(self, im, s):
        ...
        # 模态内对比学习
        im_scores = self.sim(im, im)
        s_scores = self.sim(s,s)
        margin = 0.3
        intra_loss = ((s_scores-im_scores).abs().max(0)[0]-margin).clamp(min=0).sum()
